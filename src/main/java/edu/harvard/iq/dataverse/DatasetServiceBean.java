...
public class DatasetServiceBean implements java.io.Serializable {
    ...
    */
    ...
    @Asynchronous
    public void globusAsyncjobold(Long dataset_id, String globusUserId, AuthenticatedUser authenticatedUser)  {
        ...
        try {
            Dataset dataset = find(dataset_id);
            logger.log(Level.INFO, "====dataset get id ======", dataset.getId());

            StorageIO<Dataset> datasetSIO = DataAccess.getStorageIO(dataset);
            //DatasetVersion workingVersion = dataset.getLatestVersion();
            DatasetVersion workingVersion = dataset.getEditVersion();

            logger.log(Level.INFO, "====dataset.getLatestVersion() ======", workingVersion.getId());
            logger.log(Level.INFO, "====workingVersion.isDraft() ======", workingVersion.isDraft());
            ...
            if (workingVersion.isDraft()) {
                ...
                for (FileMetadata fileMetadata : fileMetadatas) {
                    try {
                        ...
                        logger.warning("Failed to sleep for 1 seconds.");
                    }
